...
export default function WorkerAssistantPanel() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const result = await fetch(`/api/worker/assistant/${taskId}`, { cache: 'no-store' }).then(json);
        if (!active) return;
        setResponse(result.response || '');
        if (['stopped', 'error', 'waiting'].includes(result.task?.status)) {
          setBusy(false);
          setStatus(result.task.status === 'stopped' ? 'Your assistance response is ready.' : `The assistant is ${result.task.status}.`);
          return;
        }
        window.setTimeout(poll, 2500);
      } catch (cause) { if (active) { setBusy(false); setError(cause instanceof Error ? cause.message : 'Unable to read the assistance response.'); } }
    };
    poll();
    return () => { active = false; };
  }, [taskId]);
  ...
  const runWorkflow = async (event: React.FormEvent) => {
    ...
    } catch (cause) { setBusy(false); setError(cause instanceof Error ? cause.message : 'Unable to start worker assistance.'); }
  };
