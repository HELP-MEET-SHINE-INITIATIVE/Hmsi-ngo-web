...
export default function NewsroomStudio({ viewer }: { viewer: NewsViewer }) {
  ...
  const [researchScope, setResearchScope] = useState('Global humanitarian developments from the last seven days, prioritising active crises and practical humanitarian response updates.');
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const response = await fetch(`/api/admin/news/research/${researchTaskId}`, { cache: 'no-store' });
        const result = await response.json();
    ...
    poll();
    return () => { active = false; };
  }, [researchTaskId, viewer.role, loadArticles]);
  ...
  const startResearch = async () => {
    ...
    try {
      ...
      const result = await response.json();
      if (!response.ok) throw new Error(result.error || 'Unable to start humanitarian-news research.');
      setResearchTaskId(result.task?.manus_task_id || '');
      if (!result.task?.manus_task_id) setResearchBusy(false);
      setNotice('Manus is researching current humanitarian developments. Candidates will remain unpublished until reviewed.');
    } catch (researchError) { setResearchBusy(false); setError(researchError instanceof Error ? researchError.message : 'Unable to start humanitarian-news research.'); }
  };
