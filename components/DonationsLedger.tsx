...
function acknowledgementLabel(status: string | null) {
  const labels: Record<string, string> = { not_started: 'Not started', queued: 'Queued', sent: 'Sent to provider', delivered: 'Delivered', bounced: 'Bounced', failed: 'Failed', suppressed: 'Suppressed' };
...
export default function DonationsLedger() {
  ...
  return <div className="space-y-6">
    ...
    <section className="overflow-hidden rounded-3xl border border-[#d9d6ce] bg-white">
      ...
      {!loading && donations.length === 0 ? <div className="p-10 text-center"><p className="font-black">No verified donations have been recorded yet.</p><p className="mt-2 text-sm leading-6 text-[#66716a]">A donor acknowledgement cannot be sent until Paystack confirms a successful payment and HMSI records it in this protected ledger.</p></div> : <div className="overflow-x-auto"><table className="min-w-[1040px] w-full text-left text-sm"><thead className="bg-[#f6f4ef] text-[10px] font-black uppercase tracking-widest text-[#66716a]"><tr><th className="px-6 py-4">Donor</th><th className="px-6 py-4">Amount</th><th className="px-6 py-4">Channel</th><th className="px-6 py-4">Verified date</th><th className="px-6 py-4">Reference</th><th className="px-6 py-4">Acknowledgement</th><th className="px-6 py-4">Payment</th></tr></thead><tbody className="divide-y divide-[#eeeae2]">{donations.map((donation) => <tr key={donation.id}><td className="px-6 py-4"><p className="font-black">{donation.is_anonymous ? 'Anonymous donor' : donation.donor_name}</p><p className="mt-1 text-xs text-[#66716a]">{donation.donor_email}</p></td><td className="whitespace-nowrap px-6 py-4 font-black text-[#1e5b49]">{formatAmount(donation)}</td><td className="px-6 py-4 text-xs font-bold text-[#66716a]">{donation.channel || 'Not reported'}</td><td className="whitespace-nowrap px-6 py-4 text-xs font-bold text-[#66716a]">{new Date(donation.paid_at || donation.created_at).toLocaleString('en-NG')}</td><td className="px-6 py-4 font-mono text-xs text-[#66716a]">{donation.payment_reference_suffix}</td><td className="px-6 py-4"><span className={`rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-widest ${acknowledgementClass(donation.acknowledgement_status)}`}>{acknowledgementLabel(donation.acknowledgement_status)}</span>{donation.acknowledgement_updated_at ? <p className="mt-1 text-[10px] text-[#66716a]">{new Date(donation.acknowledgement_updated_at).toLocaleString('en-NG')}</p> : null}</td><td className="px-6 py-4"><span className="rounded-full bg-[#e9f0e9] px-3 py-1 text-[10px] font-black uppercase tracking-widest text-[#1e5b49]">Verified</span></td></tr>)}</tbody></table></div>}
