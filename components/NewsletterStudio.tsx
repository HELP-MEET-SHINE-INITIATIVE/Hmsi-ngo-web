...
import { CheckCircle2, Mail, RefreshCw, Send, Trash2, XCircle } from 'lucide-react';
...
export default function NewsletterStudio({ viewer, compact = false }: { viewer: Viewer; compact?: boolean }) {
  ...
  const [form, setForm] = useState({ title: '', subject: '', body: '', image_url: '', image_path: '' });
  ...
  const submitAction = async (action: string, newsletterId?: string, reason?: string) => {
    ...
    try {
      ...
      setNotice(action === 'send' ? `Newsletter sent to ${result.sentCount} active subscriber${result.sentCount === 1 ? '' : 's'}.` : action === 'delete' ? 'Newsletter draft deleted successfully.' : 'Newsletter updated successfully.');
      if (action === 'delete') setDeleteDraftId(null);
      if (action === 'reject') {
  ...
  const requestDelete = (newsletterId: string) => {
    setError('');
    setRevisionDraftId(null);
    setDeleteDraftId(newsletterId);
  };
  ...
  const createDraft = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setForm({ title: '', subject: '', body: '', image_url: '', image_path: '' });
  ...
  return (
    <section className={compact ? 'space-y-5' : 'space-y-7'}>
      ...
      {error && <div className="rounded-2xl border border-red-100 bg-red-50 p-4 text-sm text-red-700" role="alert"><p>{error}</p>{error.toLowerCase().includes('newsletter') && <p className="mt-3 flex flex-wrap items-center gap-3"><a href="https://github.com/HELP-MEET-SHINE-INITIATIVE/Hmsi-ngo-web/blob/main/supabase/newsletter_patch.sql" target="_blank" rel="noreferrer" className="font-black underline">Open newsletter SQL migration</a><button type="button" onClick={loadDrafts} className="rounded-full border border-red-200 bg-white px-3 py-1 text-[10px] font-black uppercase tracking-widest">Check again</button></p>}</div>}
