...
export default function NewsletterStudio({ viewer, compact = false }: { viewer: Viewer; compact?: boolean }) {
  ...
  return (
    <section className={compact ? 'space-y-5' : 'space-y-7'}>
      ...
      <div className="space-y-4"><div className="flex items-center justify-between gap-3"><h3 className="text-xl font-black">Newsletter drafts</h3><span className="text-xs font-bold text-[#66716a]">{drafts.length} visible</span></div>{isLoading ? <div className="rounded-3xl border border-[#d9d6ce] bg-white p-8 text-center text-sm text-[#66716a]">Loading newsletter drafts…</div> : drafts.length === 0 ? <div className="rounded-3xl border border-dashed border-[#d9d6ce] bg-white p-8 text-center text-sm italic text-[#66716a]">No newsletter drafts yet.</div> : drafts.map((draft) => <article key={draft.id} className="rounded-3xl border border-[#d9d6ce] bg-white p-6"><div className="flex flex-col justify-between gap-4 md:flex-row"><div><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-[#e9f0e9] px-3 py-1 text-[10px] font-black uppercase tracking-widest text-[#1e5b49]">{statusLabel[draft.status] || draft.status}</span><span className="rounded-full bg-[#f6f4ef] px-3 py-1 text-[10px] font-black uppercase tracking-widest text-[#66716a]">{draft.author_role}</span></div><h4 className="mt-3 text-xl font-black">{draft.title}</h4><p className="mt-1 text-sm font-bold text-[#66716a]">{draft.subject}</p><p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-[#17221e]">{draft.body}</p><p className="mt-4 text-xs text-[#66716a]">By {draft.author_name} · {new Date(draft.created_at).toLocaleString()}</p>{draft.rejection_reason && <p className="mt-3 rounded-2xl bg-red-50 p-3 text-sm text-red-700">Revision note: {draft.rejection_reason}</p>}</div><div className="flex shrink-0 flex-wrap items-start gap-2 md:max-w-[230px] md:justify-end">{viewer.role === 'worker' && draft.status === 'pending_worker_approval' && <><button disabled={isBusy} onClick={() => submitAction('approve_worker', draft.id)} className="rounded-full bg-[#1e5b49] px-4 py-2 text-xs font-black uppercase tracking-widest text-white disabled:opacity-50"><CheckCircle2 size={14} className="mr-1 inline" />Approve for admin</button><button disabled={isBusy} onClick={() => { const reason = window.prompt('Why should this volunteer draft be revised?') || ''; if (reason) submitAction('reject', draft.id, reason); }} className="rounded-full border border-red-200 px-4 py-2 text-xs font-black uppercase tracking-widest text-red-600 disabled:opacity-50"><XCircle size={14} className="mr-1 inline" />Reject</button></>}{viewer.role === 'admin' && ['draft', 'pending_worker_approval', 'pending_admin_approval', 'rejected'].includes(draft.status) && <><button disabled={isBusy} onClick={() => submitAction('approve_admin', draft.id)} className="rounded-full bg-[#1e5b49] px-4 py-2 text-xs font-black uppercase tracking-widest text-white disabled:opacity-50"><CheckCircle2 size={14} className="mr-1 inline" />Approve newsletter</button><button disabled={isBusy} onClick={() => { const reason = window.prompt('Why should this newsletter be revised?') || ''; if (reason) submitAction('reject', draft.id, reason); }} className="rounded-full border border-red-200 px-4 py-2 text-xs font-black uppercase tracking-widest text-red-600 disabled:opacity-50"><XCircle size={14} className="mr-1 inline" />Reject</button></>}{viewer.role === 'admin' && draft.status === 'approved' && <><button disabled={isBusy} onClick={() => submitAction('send', draft.id)} className="rounded-full bg-[#e1ad45] px-4 py-2 text-xs font-black uppercase tracking-widest text-[#17221e] disabled:opacity-50"><Send size={14} className="mr-1 inline" />Send now</button><button disabled={isBusy} onClick={() => { const reason = window.prompt('What should be revised before sending?') || ''; if (reason) submitAction('reject', draft.id, reason); }} className="rounded-full border border-red-200 px-4 py-2 text-xs font-black uppercase tracking-widest text-red-600 disabled:opacity-50"><XCircle size={14} className="mr-1 inline" />Request revision</button></>}</div></div></article>)}</div>
    </section>
  );
}
