...
type PromotedSharePanelProps = {
  title: string;
  description: string;
  type: 'fundraiser' | 'story';
};

export default function PromotedSharePanel({ title, description, type }: PromotedSharePanelProps) {
  const [status, setStatus] = useState('');
  const shareLabel = type === 'fundraiser' ? 'Support this verified HMSI cause' : 'Read this HMSI field story';

  const getShareUrl = () => {
    const url = new URL(window.location.href);
    url.searchParams.set('utm_source', 'social');
    url.searchParams.set('utm_medium', 'organic_share');
    url.searchParams.set('utm_campaign', type === 'fundraiser' ? 'fundraiser_promotion' : 'featured_story_promotion');
    return url.toString();
  };
