...
export default function FieldStoryFlash() {
  ...
  return <section aria-label="Approved HMSI field story flash" className="border-y border-[#cbd2ca] bg-[#e9f0e9] text-[#17221e]"><div className="mx-auto flex max-w-[1440px] flex-col gap-4 px-5 py-4 sm:px-8 lg:flex-row lg:items-center lg:justify-between lg:px-12"><div className="flex min-w-0 items-center gap-4"><div className="relative hidden h-16 w-24 shrink-0 overflow-hidden rounded-2xl bg-[#17221e] sm:block">{story.image_url ? <img src={story.image_url} alt="" className="h-full w-full object-cover" /> : <Image src="/images/outreach-10.png" alt="" fill sizes="96px" className="object-cover" />}</div><div className="min-w-0"><p className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-[#b56b3b]"><BookOpen size={14} aria-hidden="true" /> Approved field story · <RotateCw size={12} aria-hidden="true" /> rotates every 2 minutes</p><p className="mt-1 truncate text-lg font-black sm:text-xl" aria-live="polite">{story.title}</p><p className="mt-1 line-clamp-2 text-sm text-[#66716a] lg:line-clamp-1">{story.excerpt}</p><p className="mt-2 text-xs font-bold text-[#66716a]">{story.category} · By {story.author_name}</p></div></div><Link href="/#featured-stories" className="inline-flex shrink-0 items-center justify-center gap-2 rounded-full bg-[#17221e] px-5 py-3 text-xs font-black uppercase tracking-widest text-white transition hover:bg-[#1e5b49] focus:outline-none focus:ring-2 focus:ring-[#17221e] focus:ring-offset-2 focus:ring-offset-[#e9f0e9]">Read field stories <ArrowRight size={15} aria-hidden="true" /></Link></div></section>;
}
