...
export default function HomepageFeaturedStoryCard() {
  ...
  return <article id="featured-stories" className="group relative min-h-[520px] overflow-hidden rounded-3xl bg-[#17221e] text-white"><Link href={isFallback ? '/outreach/10' : `/stories/${story.id}`} className="absolute inset-0 z-10" aria-label={`Read featured story: ${story.title}`}><span className="sr-only">Read featured story: {story.title}</span></Link><img src={image} alt="" className="absolute inset-0 h-full w-full object-cover transition duration-700 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-[#17221e] via-[#17221e]/45 to-transparent" /><div className="absolute inset-x-0 bottom-0 z-20 p-7 sm:p-10"><span className="rounded-full bg-[#e1ad45] px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.15em] text-[#17221e]">Featured story</span><p className="mt-4 text-[10px] font-black uppercase tracking-[0.18em] text-white/65">{story.category}</p><h3 className="mt-3 max-w-2xl text-3xl font-black leading-tight tracking-[-0.03em] sm:text-5xl">{story.title}</h3><p className="mt-4 max-w-xl text-sm leading-6 text-white/75">{story.excerpt}</p><span className="mt-5 inline-flex text-xs font-black uppercase tracking-widest text-[#e1ad45]">Read full story <span className="ml-2" aria-hidden="true">→</span></span></div></article>;
}
