...
9. Run `supabase/fundraiser_management_patch.sql` to allow the `completed` fundraiser status used by the admin management controls. Active fundraisers remain visible on the homepage and `/impact`; completed, archived, pending, and deleted records are hidden from public fundraiser pages.
10. The newsletter and opportunity patches now include optional `image_url` and `image_path` columns. The admin, worker, and volunteer publishing forms provide an optional JPG/PNG/WEBP upload, stored securely in the existing Supabase Storage bucket. If the admin overview shows a migration warning, run `supabase/newsletter_patch.sql` and `supabase/role_opportunities_community_patch.sql` again, then redeploy.
...
The control center provides overview, fundraiser approval and management, volunteer application review, donations, contact messages, newsletter approvals, featured-story publishing, newsroom approvals, opportunity publishing, and worker assignment areas. Admins can optionally attach pictures when publishing opportunities, newsletters, field stories, newsroom articles, and fundraiser updates; approved workers and volunteers can optionally attach pictures to their newsletter, field-story, and newsroom submissions. Administrators can approve, edit, mark completed, archive, restore, or permanently delete fundraising requests; approve or reject volunteer applications; add workers; assign either assistance or job tasks with optional fundraiser links and due dates; read contact messages; reply by email; draft and approve newsletters; and send approved newsletters to active subscribers. Volunteer drafts require worker or admin review, worker drafts require admin review, and only admins can send. Run the extended `supabase/schema.sql`, `supabase/messaging_patch.sql`, `supabase/newsletter_patch.sql`, `supabase/featured_stories_patch.sql`, `supabase/newsroom_patch.sql`, and `supabase/fundraiser_management_patch.sql` in the Supabase SQL Editor before using these workflows.
