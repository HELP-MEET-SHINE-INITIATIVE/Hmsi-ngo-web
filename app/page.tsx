      ...
      <div id="main-content">
        ...
        <section className="mx-auto max-w-[1440px] px-5 py-20 sm:px-8 lg:px-12 lg:py-28">
          <div className="grid gap-10 rounded-3xl bg-[#e1ad45] p-7 sm:p-12 lg:grid-cols-[1fr_0.9fr] lg:items-center lg:p-16">
            ...
            <div className="rounded-2xl bg-[#17221e] p-7 text-white sm:p-9"><p className="text-xs font-black uppercase tracking-[0.18em] text-[#e1ad45]">Where your money goes</p><div className="mt-7 space-y-5">{[["94%", "Programmes & direct support", "bg-[#e1ad45]"], ["4%", "Community accountability", "bg-[#6aa58c]"], ["2%", "Operations & learning", "bg-white/50"]].map(([value, label, color]) => <div key={label}><div className="flex justify-between text-sm font-bold"><span>{label}</span><span className="text-[#e1ad45]">{value}</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-white/10"><div className={`h-full rounded-full ${color}`} style={{ width: value }} /></div></div>)}</div><Link href="/donate" className="mt-8 flex items-center justify-center gap-2 rounded-full bg-[#e1ad45] px-5 py-4 text-sm font-black uppercase tracking-[0.13em] text-[#17221e] transition hover:bg-white">Donate now <ArrowUpRight size={17} /></Link></div>
          </div>
        </section>
