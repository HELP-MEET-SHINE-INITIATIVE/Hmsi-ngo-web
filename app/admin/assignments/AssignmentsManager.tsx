...
type Worker = { id: string; name: string; email: string; phone: string; status: string; onboarding_status?: string };
...
export default function AssignmentsManager() {
  ...
  return <div className="space-y-6">
    <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-[#b56b3b]">Operations menu</p><h1 className="mt-2 text-3xl font-black tracking-tight">Job assignments</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[#66716a]">Review who owns each job, inspect the brief, update the assignment, or move it into the 30-day recovery process.</p></div><Link href="/hmsi-control?view=assignments" className="inline-flex items-center justify-center gap-2 rounded-full bg-[#1e5b49] px-4 py-3 text-xs font-black uppercase tracking-widest text-white"><Users size={15} /> Create assignment</Link></div>
    ...
    {visible.length === 0 ? <div className="rounded-3xl border border-dashed border-[#d9d6ce] bg-white p-10 text-center text-sm text-[#66716a]">No active assignments match this view.</div> : <div className="grid gap-4">{visible.map((item) => <article key={item.id} className="rounded-3xl border border-[#d9d6ce] bg-white p-5 sm:p-6"><div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap gap-2 text-[10px] font-black uppercase tracking-widest"><span className="rounded-full bg-[#f6f4ef] px-3 py-1 text-[#b56b3b]">{item.kind}</span><span className="rounded-full bg-[#e9f0e9] px-3 py-1 text-[#1e5b49]">{item.status.replace('_', ' ')}</span></div><h2 className="mt-3 text-xl font-black">{item.title}</h2><p className="mt-2 text-sm leading-6 text-[#66716a]">{item.description}</p></div><div className="flex shrink-0 flex-wrap gap-2"><button type="button" onClick={() => beginEdit(item)} className="inline-flex items-center gap-2 rounded-full border border-[#d9d6ce] px-4 py-2 text-xs font-black uppercase tracking-widest text-[#1e5b49]"><Edit3 size={14} /> Edit</button><button type="button" onClick={() => setConfirming(item)} className="inline-flex items-center gap-2 rounded-full border border-red-200 px-4 py-2 text-xs font-black uppercase tracking-widest text-red-700"><Trash2 size={14} /> Delete</button></div></div><div className="mt-5 grid gap-3 border-t border-[#ece8df] pt-5 sm:grid-cols-3"><div><p className="text-[10px] font-black uppercase tracking-widest text-[#66716a]">Assigned to</p><p className="mt-1 font-black">{item.assigned_worker_name || 'Worker record unavailable'}</p><p className="mt-1 break-all text-xs text-[#66716a]">{item.assigned_worker_email || 'No email available'}</p></div><div><p className="text-[10px] font-black uppercase tracking-widest text-[#66716a]">Due</p><p className="mt-1 font-black">{item.due_at ? new Date(item.due_at).toLocaleString('en-NG') : 'No due date'}</p></div><div><p className="text-[10px] font-black uppercase tracking-widest text-[#66716a]">Created</p><p className="mt-1 font-black">{new Date(item.created_at).toLocaleDateString('en-NG')}</p></div></div>{item.admin_note && <p className="mt-4 rounded-2xl bg-[#fff8e8] p-3 text-xs leading-5 text-[#7a5b16]"><strong>Admin note:</strong> {item.admin_note}</p>}</article>)}</div>}
    {editing && <div className="fixed inset-0 z-50 flex items-end justify-center bg-[#17221e]/50 p-3 sm:items-center"><form onSubmit={saveEdit} className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-3xl bg-white p-6 shadow-2xl sm:p-8"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-widest text-[#b56b3b]">Review assignment</p><h2 className="mt-2 text-2xl font-black">Edit job details</h2></div><button type="button" onClick={() => setEditing(null)} aria-label="Close edit dialog" className="rounded-full p-2 text-[#66716a] hover:bg-[#f6f4ef]"><X size={20} /></button></div><div className="mt-6 space-y-4"><input required value={editForm.title} onChange={(event) => setEditForm({ ...editForm, title: event.target.value })} placeholder="Assignment title" className="w-full rounded-2xl bg-[#f6f4ef] px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-[#1e5b49]" /><textarea required rows={5} value={editForm.description} onChange={(event) => setEditForm({ ...editForm, description: event.target.value })} placeholder="Assignment brief" className="w-full resize-none rounded-2xl bg-[#f6f4ef] px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-[#1e5b49]" /><div className="grid gap-4 sm:grid-cols-2"><select value={editForm.kind} onChange={(event) => setEditForm({ ...editForm, kind: event.target.value })} className="rounded-2xl bg-[#f6f4ef] px-4 py-3 text-sm outline-none"><option value="assistance">Assistance</option><option value="job">Job</option></select><select value={editForm.status} onChange={(event) => setEditForm({ ...editForm, status: event.target.value })} className="rounded-2xl bg-[#f6f4ef] px-4 py-3 text-sm outline-none">{statuses.map((status) => <option key={status} value={status}>{status.replace('_', ' ')}</option>)}</select></div><select required value={editForm.workerId} onChange={(event) => setEditForm({ ...editForm, workerId: event.target.value })} className="w-full rounded-2xl bg-[#f6f4ef] px-4 py-3 text-sm outline-none"><option value="">Select completed worker</option>{workers.filter((worker) => worker.status === 'active' && worker.onboarding_status === 'completed').map((worker) => <option key={worker.id} value={worker.id}>{worker.name} · {worker.email}</option>)}</select><label className="block text-xs font-black uppercase tracking-widest text-[#66716a]">Due date<input type="datetime-local" value={editForm.dueAt} onChange={(event) => setEditForm({ ...editForm, dueAt: event.target.value })} className="mt-2 w-full rounded-2xl bg-[#f6f4ef] px-4 py-3 text-sm font-normal normal-case tracking-normal outline-none" /></label><textarea rows={3} value={editForm.adminNote} onChange={(event) => setEditForm({ ...editForm, adminNote: event.target.value })} placeholder="Internal administrator note (optional)" className="w-full resize-none rounded-2xl bg-[#f6f4ef] px-4 py-3 text-sm outline-none" /></div><div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><button type="button" onClick={() => setEditing(null)} className="rounded-full border border-[#d9d6ce] px-5 py-3 text-xs font-black uppercase tracking-widest text-[#66716a]">Cancel</button><button disabled={savingId === editing.id} className="inline-flex items-center justify-center gap-2 rounded-full bg-[#1e5b49] px-5 py-3 text-xs font-black uppercase tracking-widest text-white disabled:opacity-60">{savingId === editing.id && <Loader2 className="animate-spin" size={15} />} Save changes</button></div></form></div>}
