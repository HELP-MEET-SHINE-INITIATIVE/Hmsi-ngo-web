...
type Volunteer = { id: string; name: string; email: string; phone: string; interest: string; status: string; account_status: string; auth_user_id: string | null; assignment_ready: boolean; portal_ready: boolean };
...
export default function VolunteerAssignmentsManager() {
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-3xl border border-[#d9d6ce] bg-white p-6"><div className="flex items-start gap-3"><Users className="mt-1 text-[#1e5b49]" size={20} /><div><h2 className="font-black">Issue a volunteer assignment</h2><p className="mt-1 text-sm leading-6 text-[#66716a]">Every approved active volunteer is selectable. If portal activation is pending, the assignment is held securely and no task email is sent until the volunteer completes the official onboarding invitation.</p></div></div><form onSubmit={createAssignment} className="mt-5 grid gap-4 md:grid-cols-2"><input required value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} placeholder="Assignment title" className="rounded-2xl bg-[#f6f4ef] px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-[#1e5b49] md:col-span-2" /><textarea required value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} rows={4} placeholder="Required outcome, safety context, and expected completion" className="resize-none rounded-2xl bg-[#f6f4ef] px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-[#1e5b49] md:col-span-2" /><select required value={form.volunteerId} onChange={(event) => setForm({ ...form, volunteerId: event.target.value })} className="rounded-2xl bg-[#f6f4ef] px-4 py-3 text-sm outline-none"><option value="">Select an approved volunteer…</option>{assignmentReady.map((volunteer) => <option key={volunteer.id} value={volunteer.id}>{volunteer.name} · {volunteer.email} · {volunteer.interest} · {volunteer.portal_ready ? 'portal ready' : 'activation pending'}</option>)}</select><select value={form.category} onChange={(event) => setForm({ ...form, category: event.target.value })} className="rounded-2xl bg-[#f6f4ef] px-4 py-3 text-sm outline-none">{categories.map((category) => <option key={category} value={category}>{humanize(category)}</option>)}</select><select value={form.priority} onChange={(event) => setForm({ ...form, priority: event.target.value })} className="rounded-2xl bg-[#f6f4ef] px-4 py-3 text-sm outline-none">{priorities.map((priority) => <option key={priority} value={priority}>{priority}</option>)}</select><label className="text-xs font-black uppercase tracking-widest text-[#66716a]">Due date<input type="datetime-local" value={form.dueAt} onChange={(event) => setForm({ ...form, dueAt: event.target.value })} className="mt-2 w-full rounded-2xl bg-[#f6f4ef] px-4 py-3 text-sm font-normal normal-case tracking-normal outline-none" /></label><label className="flex items-center gap-3 rounded-2xl bg-[#f6f4ef] px-4 py-3 text-sm font-bold text-[#17221e] md:col-span-2"><input type="checkbox" checked={form.proofRequired} onChange={(event) => setForm({ ...form, proofRequired: event.target.checked })} /> Require a private Google Drive or Docs proof link before review</label><textarea value={form.adminNote} onChange={(event) => setForm({ ...form, adminNote: event.target.value })} rows={2} placeholder="Internal admin note (not visible in the volunteer workspace)" className="resize-none rounded-2xl bg-[#f6f4ef] px-4 py-3 text-sm outline-none md:col-span-2" /><button disabled={saving || assignmentReady.length === 0} className="inline-flex items-center justify-center gap-2 rounded-full bg-[#1e5b49] px-5 py-3 text-xs font-black uppercase tracking-widest text-white disabled:opacity-50 md:col-span-2">{saving && <Loader2 size={15} className="animate-spin" />} Create assignment</button></form>{assignmentReady.length === 0 && <p className="mt-4 rounded-2xl bg-[#fff8e8] p-4 text-xs leading-5 text-[#7a5b16]">No approved active volunteers are ready for an assignment yet. Approve the volunteer application or restore its active access first.</p>}</section>
