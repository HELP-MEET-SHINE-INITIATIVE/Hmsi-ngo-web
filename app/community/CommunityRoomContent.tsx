...
export default function CommunityRoomContent({ room }: { room: Room }) {
  ...
  useEffect(() => {
    fetch('/api/sponsorships', { cache: 'no-store' }).then((response) => response.ok ? response.json() : null).then((payload) => setSponsorships(payload?.sponsorships || [])).catch(() => setSponsorships([]));
  }, []);
  ...
  const memberCount = new Set(posts.map((post) => post.author_name)).size;
  const totalLikes = posts.reduce((sum, post) => sum + Number(post.likeCount || 0), 0);
  const totalComments = posts.reduce((sum, post) => sum + Number(post.comments?.length || 0), 0);
  const activityScore = Math.min(100, posts.length * 10 + totalLikes * 2 + totalComments * 3);

  return <main className="min-h-screen bg-[#f6f4ef] px-6 py-12 text-[#17221e]"><div className="mx-auto max-w-3xl"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.18em] text-[#b56b3b]"><Users size={16} />{room === 'volunteer' ? 'Volunteer room' : 'Worker room'}</p><h1 className="mt-2 text-4xl font-black tracking-tight">{room === 'volunteer' ? 'Volunteer collaboration' : 'Worker coordination'}</h1><p className="mt-3 text-[#66716a]">{room === 'volunteer' ? 'Share ideas, coordinate outreach, and collaborate with volunteers and workers.' : 'Coordinate tasks, field updates, and worker-only collaboration.'}</p></div><div className="flex flex-wrap gap-2">{viewer.role === 'admin' && <Link href="/hmsi-control" className="rounded-full bg-[#1e5b49] px-4 py-3 text-xs font-black uppercase tracking-widest text-white">Admin dashboard</Link>}<Link href="/opportunities" className="rounded-full border border-[#d9d6ce] bg-white px-4 py-3 text-xs font-black uppercase tracking-widest">Opportunities</Link>{room === 'worker' ? <Link href="/volunteer-room" className="rounded-full border border-[#d9d6ce] bg-white px-4 py-3 text-xs font-black uppercase tracking-widest">Volunteer room</Link> : viewer.role === 'worker' || viewer.role === 'admin' ? <Link href="/worker-room" className="rounded-full border border-[#d9d6ce] bg-white px-4 py-3 text-xs font-black uppercase tracking-widest">Worker room</Link> : null}</div></div><HmsiRoomFlashPlacements />{room === 'volunteer' && viewer.role === 'volunteer' && <div className="mt-6 space-y-6"><NewsroomStudio viewer={{ email: viewer.email, name: viewer.name, role: 'volunteer' }} /><FeaturedStoryStudio viewer={{ email: viewer.email, name: viewer.name, role: 'volunteer' }} /></div>}<section className="mt-8 grid gap-4 sm:grid-cols-4"><div className="rounded-3xl border border-[#d9d6ce] bg-white p-5"><p className="text-2xl font-black text-[#1e5b49]">{posts.length}</p><p className="mt-1 text-[10px] font-black uppercase tracking-widest text-[#66716a]">Posts</p></div><div className="rounded-3xl border border-[#d9d6ce] bg-white p-5"><p className="text-2xl font-black text-[#1e5b49]">{memberCount}</p><p className="mt-1 text-[10px] font-black uppercase tracking-widest text-[#66716a]">Contributors</p></div><div className="rounded-3xl border border-[#d9d6ce] bg-white p-5"><p className="text-2xl font-black text-[#1e5b49]">{totalLikes + totalComments}</p><p className="mt-1 text-[10px] font-black uppercase tracking-widest text-[#66716a]">Interactions</p></div><div className="rounded-3xl border border-[#d9d6ce] bg-white p-5"><p className="text-2xl font-black text-[#1e5b49]">{activityScore}%</p><p className="mt-1 text-[10px] font-black uppercase tracking-widest text-[#66716a]">Activity signal</p><div className="mt-3 h-2 rounded-full bg-[#e9f0e9]"><div className="h-2 rounded-full bg-[#e1ad45]" style={{ width: `${activityScore}%` }} /></div></div></section>{sponsorships.length > 0 && <section className="mt-6 rounded-3xl border border-[#ead9ad] bg-[#fff8e8] p-5"><div className="flex items-center justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-widest text-[#b56b3b]">Sponsored community information</p><h2 className="mt-2 text-lg font-black">Partner messages reviewed by HMSI.</h2></div><Link href="/sponsor" className="text-[10px] font-black uppercase tracking-widest text-[#1e5b49] hover:underline">Sponsor a placement</Link></div><div className="mt-4 grid gap-3 md:grid-cols-2">{sponsorships.slice(0, 4).map((sponsorship) => <a key={sponsorship.id} href={sponsorship.target_url} target="_blank" rel="noreferrer" className="rounded-2xl border border-[#ead9ad] bg-white p-4 transition hover:border-[#1e5b49]"><p className="text-[10px] font-black uppercase tracking-widest text-[#b56b3b]">Sponsored · {sponsorship.organisation_name || 'Partner'}</p><h3 className="mt-2 font-black">{sponsorship.title}</h3><p className="mt-1 text-xs leading-5 text-[#66716a]">{sponsorship.description}</p></a>)}</div></section>}{status && <div className="mt-6 rounded-2xl border border-red-100 bg-red-50 p-4 text-sm text-red-700">{status}</div>}<form onSubmit={createPost} className="mt-8 rounded-3xl border border-[#d9d6ce] bg-white p-6"><textarea required value={draft} onChange={(event) => setDraft(event.target.value)} rows={3} placeholder={room === 'worker' ? 'Share a worker update…' : 'Share an opportunity, idea, or progress update…'} className="w-full resize-none rounded-2xl bg-[#f6f4ef] p-4 text-sm outline-none focus:ring-2 focus:ring-[#1e5b49]" /><button disabled={isBusy} className="mt-4 flex items-center gap-2 rounded-full bg-[#1e5b49] px-5 py-3 text-xs font-black uppercase tracking-widest text-white disabled:opacity-50"><Send size={15} />{isBusy ? 'Publishing…' : 'Publish post'}</button></form><div className="mt-8 space-y-5">{posts.length === 0 ? <div className="rounded-3xl border border-dashed border-[#d9d6ce] bg-white p-10 text-center text-[#66716a]">No posts yet. Start the conversation.</div> : posts.map((post) => <article key={post.id} className="rounded-3xl border border-[#d9d6ce] bg-white p-6"><div className="flex items-center justify-between"><div><p className="font-black">{post.author_name}</p><p className="text-xs font-bold uppercase tracking-widest text-[#b56b3b]">{post.author_role} · {new Date(post.created_at).toLocaleString()}</p></div></div><p className="mt-4 leading-7">{post.content}</p><div className="mt-5 flex items-center gap-5 border-t border-[#f6f4ef] pt-4"><button onClick={() => toggleLike(post)} className={`flex items-center gap-2 text-xs font-black ${post.likedByMe ? 'text-[#1e5b49]' : 'text-[#66716a]'}`}><Heart size={16} fill={post.likedByMe ? 'currentColor' : 'none'} />Like {post.likeCount || 0}</button><span className="flex items-center gap-2 text-xs font-black text-[#66716a]"><MessageCircle size={16} />{post.comments?.length || 0} comments</span></div><form onSubmit={(event) => addComment(event, post.id)} className="mt-4 flex gap-2"><input value={commentDraft[post.id] || ''} onChange={(event) => setCommentDraft({ ...commentDraft, [post.id]: event.target.value })} placeholder="Write a comment…" className="min-w-0 flex-1 rounded-full bg-[#f6f4ef] px-4 py-2 text-xs outline-none" /><button className="rounded-full bg-[#17221e] px-4 py-2 text-[10px] font-black uppercase tracking-widest text-white">Comment</button></form>{post.comments?.length > 0 && <div className="mt-4 space-y-2 border-t border-[#f6f4ef] pt-4">{post.comments.map((comment: any) => <div key={comment.id} className="rounded-2xl bg-[#f6f4ef] p-3"><div className="flex items-start justify-between gap-2"><p className="text-xs font-black">{comment.author_name}</p>{(viewer.role === 'admin' || (viewer.role === 'worker' && room === 'volunteer')) && <button type="button" onClick={() => moderateComment(comment.id)} className="text-[10px] font-black uppercase tracking-widest text-red-600">{viewer.role === 'admin' ? 'Delete' : 'Flag'}</button>}</div><p className="mt-1 text-sm">{comment.content}</p></div>)}</div>}</article>)}</div></div></main>;
}
