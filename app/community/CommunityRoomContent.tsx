...
export default function CommunityRoomContent({ room }: { room: Room }) {
  ...
  useEffect(() => {
    if (!authLoading && user) loadPosts().catch((error) => setStatus(error instanceof Error ? error.message : 'Community posts are unavailable.')).finally(() => setIsLoading(false));
    if (!authLoading && !user) setIsLoading(false);
  }, [authLoading, user, room, loadPosts]);
  ...
  if (!user) return <main className="flex min-h-screen items-center justify-center bg-[#f6f4ef] p-6"><div className="rounded-3xl bg-white p-8 text-center shadow-xl"><h1 className="text-2xl font-black">Sign in to enter the community</h1><Link href="/login" className="mt-5 inline-block rounded-full bg-[#1e5b49] px-5 py-3 text-xs font-black uppercase tracking-widest text-white">Sign in</Link></div></main>;
  if (room === 'worker' && user.role !== 'worker') return <main className="flex min-h-screen items-center justify-center bg-[#f6f4ef] p-6"><div className="rounded-3xl bg-white p-8 text-center shadow-xl"><h1 className="text-2xl font-black">Worker room access required</h1><p className="mt-3 text-sm text-[#66716a]">This space is for approved workers and coordinators.</p><Link href="/volunteer-room" className="mt-5 inline-block rounded-full bg-[#1e5b49] px-5 py-3 text-xs font-black uppercase tracking-widest text-white">Open volunteer room</Link></div></main>;
