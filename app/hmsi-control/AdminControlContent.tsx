...
import { AlertCircle, Calendar, CheckCircle2, CircleDollarSign, ClipboardList, LogOut, ShieldCheck, UserPlus, Users, XCircle } from 'lucide-react';
...
export default function AdminControlContent() {
  ...
  const [activeView, setActiveView] = useState<'overview' | 'fundraisers' | 'volunteers' | 'opportunities' | 'donations' | 'messages' | 'newsletter' | 'stories' | 'assignments'>('overview');
  ...
  const [opportunityForm, setOpportunityForm] = useState({ title: '', description: '', audience: 'volunteer', location: 'Nigeria and Africa', startsAt: '', endsAt: '' });
  ...
  const createOpportunity = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setOpportunityForm({ title: '', description: '', audience: 'volunteer', location: 'Nigeria and Africa', startsAt: '', endsAt: '' });
      await loadOverview();
    ...
    <div className="min-h-screen bg-[#f6f4ef] text-[#17221e]">
      ...
      <main className="mx-auto grid max-w-7xl gap-8 px-6 py-8 lg:grid-cols-[230px_1fr]">
        ...
        <section className="min-w-0 space-y-6">
          ...
          {activeView === 'fundraisers' && <div className="space-y-4"><SectionHeading title="Fundraising approvals" subtitle="Approve verified requests so they become visible publicly, or permanently delete rejected requests and their uploaded images." />{data.fundraisers.length === 0 ? <Empty text="No fundraiser records yet." /> : data.fundraisers.map((fundraiser) => <div key={fundraiser.id} className="rounded-3xl border border-[#d9d6ce] bg-white p-6"><div className="relative mb-6 h-48 overflow-hidden rounded-2xl bg-[#f6f4ef]"><Image src={fundraiser.image_url || '/images/outreach-1.png'} alt={`Cover image for ${fundraiser.title}`} fill sizes="(max-width: 768px) 100vw, 720px" className="object-cover" /></div><div className="flex flex-col justify-between gap-4 md:flex-row"><div><div className="mb-2 flex flex-wrap items-center gap-2"><span className="rounded-full bg-[#e9f0e9] px-3 py-1 text-[10px] font-black uppercase tracking-widest text-[#1e5b49]">{fundraiser.category}</span><span className="rounded-full bg-[#f6f4ef] px-3 py-1 text-[10px] font-black uppercase tracking-widest text-[#66716a]">{fundraiser.status}</span></div><h3 className="text-xl font-black">{fundraiser.title}</h3><p className="mt-2 max-w-2xl text-sm leading-6 text-[#66716a]">{fundraiser.description}</p><p className="mt-3 text-xs font-bold text-[#66716a]">Target: ₦{Number(fundraiser.target_amount).toLocaleString()} · {new Date(fundraiser.created_at).toLocaleDateString()}</p></div><div className="flex shrink-0 flex-wrap items-start gap-2"><button disabled={isBusy} onClick={() => updateRecord(`/api/admin/fundraisers/${fundraiser.id}`, { status: 'active' })} className="rounded-full bg-[#1e5b49] px-4 py-2 text-xs font-black uppercase tracking-widest text-white disabled:opacity-50"><CheckCircle2 size={14} className="mr-1 inline" />Approve</button><button disabled={isBusy} onClick={() => updateRecord(`/api/admin/fundraisers/${fundraiser.id}`, { status: 'rejected' })} className="rounded-full border border-red-200 px-4 py-2 text-xs font-black uppercase tracking-widest text-red-600 disabled:opacity-50"><XCircle size={14} className="mr-1 inline" />Reject</button></div></div></div>)}</div>}
