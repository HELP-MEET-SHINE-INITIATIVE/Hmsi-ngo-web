...
type AdminData = {
  ...
  donations: Array<any>;
  donationSummary: { count: number; totalAmountNgn: number; totalAmountByCurrency?: Record<string, number> };
};

const emptyData: AdminData = { fundraisers: [], volunteers: [], workers: [], assignments: [], opportunities: [], opportunityApplications: [], donations: [], donationSummary: { count: 0, totalAmountNgn: 0, totalAmountByCurrency: {} } };
    ...
    <div className="min-h-screen bg-[#f6f4ef] text-[#17221e]">
      ...
      <main className="mx-auto grid max-w-7xl gap-8 px-6 py-8 lg:grid-cols-[230px_1fr]">
        ...
        <section className="min-w-0 space-y-6">
          ...
          {activeView === 'donations' && <div className="space-y-6"><SectionHeading title="Donations ledger" subtitle="Verified Paystack payments recorded by HMSI, including general donations and fundraiser gifts." /><div className="grid gap-4 sm:grid-cols-2"><div className="rounded-3xl border border-[#d9d6ce] bg-white p-6"><div className="flex items-center gap-3 text-[#1e5b49]"><CircleDollarSign size={22} /><p className="text-xs font-black uppercase tracking-widest">Total verified donations</p></div><p className="mt-4 text-3xl font-black">₦{Number(data.donationSummary.totalAmountNgn || 0).toLocaleString('en-NG', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>{Number(data.donationSummary.totalAmountByCurrency?.USD || 0) > 0 && <p className="mt-2 text-lg font-black text-[#1e5b49]">USD {Number(data.donationSummary.totalAmountByCurrency?.USD || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>}<p className="mt-2 text-xs text-[#66716a]">Successful Paystack transactions, shown separately by currency. No exchange-rate conversion is implied.</p></div><StatCard label="Successful payments" value={data.donationSummary.count} /></div><div className="overflow-hidden rounded-3xl border border-[#d9d6ce] bg-white"><div className="border-b border-[#d9d6ce] px-6 py-5"><h3 className="text-xl font-black">Recent transactions</h3><p className="mt-1 text-sm text-[#66716a]">Showing the latest {data.donations.length} recorded donation(s).</p></div>{data.donations.length === 0 ? <div className="p-10"><Empty text="No verified donations have been recorded yet. Run supabase/donations_patch.sql, then accept a Paystack payment." /></div> : <div className="overflow-x-auto"><table className="min-w-[920px] w-full text-left text-sm"><thead className="bg-[#f6f4ef] text-[10px] font-black uppercase tracking-widest text-[#66716a]"><tr><th className="px-6 py-4">Donor</th><th className="px-6 py-4">Amount</th><th className="px-6 py-4">Fundraiser</th><th className="px-6 py-4">Date</th><th className="px-6 py-4">Paystack reference</th><th className="px-6 py-4">Status</th></tr></thead><tbody className="divide-y divide-[#eeeae2]">{data.donations.map((donation) => <tr key={donation.id} className="align-top"><td className="px-6 py-4"><p className="font-black">{donation.donor_name}</p><p className="mt-1 text-xs text-[#66716a]">{donation.donor_email}</p></td><td className="whitespace-nowrap px-6 py-4 font-black text-[#1e5b49]">{donation.currency || 'NGN'} {Number(donation.amount_major ?? donation.amount_ngn ?? 0).toLocaleString(donation.currency === 'USD' ? 'en-US' : 'en-NG', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td><td className="px-6 py-4 text-xs font-bold text-[#66716a]">{data.fundraisers.find((fundraiser) => fundraiser.id === donation.fundraiser_id)?.title || 'General HMSI donation'}</td><td className="whitespace-nowrap px-6 py-4 text-xs font-bold text-[#66716a]">{new Date(donation.paid_at || donation.created_at).toLocaleString('en-NG')}</td><td className="max-w-[180px] break-all px-6 py-4 font-mono text-xs text-[#66716a]">{donation.paystack_reference}</td><td className="px-6 py-4"><span className={`rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-widest ${donation.status === 'success' ? 'bg-[#e9f0e9] text-[#1e5b49]' : 'bg-[#f6f4ef] text-[#66716a]'}`}>{donation.status}</span></td></tr>)}</tbody></table></div>}</div></div>}
