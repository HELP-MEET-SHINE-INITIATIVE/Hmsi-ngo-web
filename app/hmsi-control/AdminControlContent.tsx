  ...
  return (
    <div className="min-h-screen bg-[#f6f4ef] text-[#17221e]">
      ...
      <main className="mx-auto grid max-w-7xl gap-8 px-6 py-8 lg:grid-cols-[230px_1fr]">
        <aside><nav className="sticky top-6 space-y-2 rounded-3xl border border-[#d9d6ce] bg-white p-3">{navItems.map((item) => <button key={item.id} onClick={() => setActiveView(item.id)} className={`w-full rounded-2xl px-4 py-3 text-left text-xs font-black uppercase tracking-widest transition ${activeView === item.id ? 'bg-[#1e5b49] text-white' : 'text-[#66716a] hover:bg-[#f6f4ef]'}`}>{item.label}</button>)}<div className="mt-3 border-t border-[#f6f4ef] pt-3"><p className="px-3 pb-2 text-[10px] font-black uppercase tracking-[0.17em] text-[#b56b3b]">Editorial</p><Link href="/admin/editorial" className="block rounded-2xl px-4 py-3 text-xs font-black uppercase tracking-widest text-[#66716a] hover:bg-[#f6f4ef]">Editorial Queue</Link><Link href="/admin/articles" className="block rounded-2xl px-4 py-3 text-xs font-black uppercase tracking-widest text-[#66716a] hover:bg-[#f6f4ef]">Content Management</Link></div></nav></aside>
