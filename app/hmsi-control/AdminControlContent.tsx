...
import { AlertCircle, CheckCircle2, ClipboardList, LogOut, ShieldCheck, UserPlus, Users, XCircle } from 'lucide-react';
...
const emptyData: AdminData = { fundraisers: [], volunteers: [], workers: [], assignments: [] };

export default function AdminControlContent() {
  const [sessionChecked, setSessionChecked] = useState(false);
  const [authenticated, setAuthenticated] = useState(false);
  const [data, setData] = useState<AdminData>(emptyData);
  const [activeView, setActiveView] = useState<'overview' | 'fundraisers' | 'volunteers' | 'assignments'>('overview');
  ...
  const updateRecord = async (url: string, body: Record<string, string>) => {
    ...
    try {
      ...
      await loadOverview();
      setNotice(body.status === 'rejected' ? 'Fundraiser and uploaded image permanently deleted.' : 'Update saved.');
    } catch (actionError) {
    ...
    <div className="min-h-screen bg-[#f6f4ef] text-[#17221e]">
      ...
      <main className="mx-auto grid max-w-7xl gap-8 px-6 py-8 lg:grid-cols-[230px_1fr]">
        ...
        <section className="min-w-0 space-y-6">
          ...
          {activeView === 'overview' && <div className="space-y-6"><div className="grid grid-cols-2 gap-4 lg:grid-cols-4"><StatCard label="Pending fundraisers" value={pendingFundraisers.length} /><StatCard label="Pending volunteers" value={pendingVolunteers.length} /><StatCard label="Active workers" value={data.workers.filter((worker) => worker.status === 'active').length} /><StatCard label="Open assignments" value={data.assignments.filter((assignment) => assignment.status !== 'completed').length} /></div><div className="rounded-3xl border border-[#d9d6ce] bg-white p-6"><h2 className="mb-2 text-xl font-black">Today’s queue</h2><p className="mb-6 text-sm text-[#66716a]">Review new help requests and volunteer applications, then assign clear next steps to a worker.</p><div className="grid gap-4 md:grid-cols-2"><QueueCard title="Fundraising approvals" count={pendingFundraisers.length} onClick={() => setActiveView('fundraisers')} /><QueueCard title="Volunteer applications" count={pendingVolunteers.length} onClick={() => setActiveView('volunteers')} /></div></div></div>}
          ...
          {activeView === 'volunteers' && <div className="space-y-4"><SectionHeading title="Volunteer applications" subtitle="Approve trusted volunteers, reject unsuitable applications, and add approved people to the worker directory when ready." />{data.volunteers.length === 0 ? <Empty text="No volunteer applications yet." /> : data.volunteers.map((volunteer) => <div key={volunteer.id} className="rounded-3xl border border-[#d9d6ce] bg-white p-6"><div className="flex flex-col justify-between gap-4 md:flex-row"><div><div className="mb-2 flex items-center gap-2"><span className="rounded-full bg-[#f6f4ef] px-3 py-1 text-[10px] font-black uppercase tracking-widest text-[#66716a]">{volunteer.status}</span><span className="text-xs font-bold text-[#b56b3b]">{volunteer.interest}</span></div><h3 className="text-xl font-black">{volunteer.name}</h3><p className="mt-1 text-sm text-[#66716a]">{volunteer.email} · {volunteer.phone}</p><p className="mt-3 max-w-2xl text-sm leading-6 text-[#17221e]">{volunteer.message}</p></div><div className="flex shrink-0 flex-wrap items-start gap-2"><button disabled={isBusy} onClick={() => updateRecord(`/api/admin/volunteers/${volunteer.id}`, { status: 'approved' })} className="rounded-full bg-[#1e5b49] px-4 py-2 text-xs font-black uppercase tracking-widest text-white disabled:opacity-50">Approve</button><button disabled={isBusy} onClick={() => updateRecord(`/api/admin/volunteers/${volunteer.id}`, { status: 'rejected' })} className="rounded-full border border-red-200 px-4 py-2 text-xs font-black uppercase tracking-widest text-red-600 disabled:opacity-50">Reject</button></div></div></div>)}</div>}
