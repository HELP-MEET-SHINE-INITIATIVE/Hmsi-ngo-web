...
import { ArrowRight, CheckCircle2, Clock3, MailCheck, ShieldCheck } from 'lucide-react';
...
export default function OnboardingContent({ token }: { token: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setSetupEmailSent(Boolean(payload.setupEmailSent));
    } catch (loadError) {
  ...
  const updateTask = async (task: Task) => {
    ...
    try {
      ...
      if (payload.setupEmailSent) setSetupEmailSent(true);
    } catch (updateError) {
