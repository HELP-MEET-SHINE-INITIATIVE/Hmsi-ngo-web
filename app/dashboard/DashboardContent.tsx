...
export default function DashboardContent() {
  ...
  useEffect(() => {
    ...
    setData(loadData());
    setIsLoading(false);
  }, [viewer, authLoading, adminCheckLoading, router]);

  useEffect(() => {
    if (!viewer || viewer.role !== 'worker' || viewer.email === 'admin') return;
    fetch(`/api/messages?email=${encodeURIComponent(viewer.email)}&role=worker`, { cache: 'no-store' })
      .then((response) => response.json())
      .then((result) => setMessageUnreadCount(Number(result.unreadCount || 0)))
      .catch(() => setMessageUnreadCount(0));
  }, [viewer]);

  if (authLoading || adminCheckLoading || isLoading || !viewer) {
    return (
  ...
  const userProjects = data.projects.filter((p: any) =>
    p.lead === viewer.name || p.volunteers.includes(viewer.id)
  );
  ...
  const handleCreatePost = (event: React.FormEvent) => {
    ...
    const nextData = {
      ...data,
      activities: [{ id: `post-${Date.now()}`, userId: viewer.id, userName: viewer.name, type: 'update', content, timestamp: new Date().toISOString(), comments: [], likes: 0 }, ...data.activities],
    };
    saveData(nextData);
    setData(nextData);
    setPostDraft('');
  ...
  const handleComment = (event: React.FormEvent, activityId: string) => {
    ...
    if (!text) return;
    const nextData = { ...data, activities: data.activities.map((activity: any) => activity.id === activityId ? { ...activity, comments: [...(activity.comments || []), { id: `comment-${Date.now()}`, userName: viewer.name, text }] } : activity) };
    saveData(nextData);
    setData(nextData);
  ...
  const handleViewerLogout = async () => {
    if (viewer?.role === 'admin') {
      await fetch('/api/admin/session', { method: 'DELETE' });
      router.push('/hmsi-control');
      return;
    }
    logout();
  };
      ...
      <header className="sticky top-0 z-40 bg-white border-b border-[#d9d6ce] px-6 py-3">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => viewer.role === 'worker' ? setActivePanel('messages') : router.push('/volunteer-room')} className="p-2 rounded-full hover:bg-[#f6f4ef] text-[#66716a]" aria-label={viewer.role === 'worker' ? 'Open messages' : 'Open volunteer room'}>
              <MessageSquare size={20} />
            </button>
            {(viewer.role === 'worker' || viewer.role === 'volunteer') && <button onClick={() => setActivePanel('newsletter')} className="p-2 rounded-full hover:bg-[#f6f4ef] text-[#66716a]" aria-label="Open newsletter studio"><Mail size={20} /></button>}
            {(viewer.role === 'worker' || viewer.role === 'volunteer') && <button onClick={() => setActivePanel('stories')} className="p-2 rounded-full hover:bg-[#f6f4ef] text-[#66716a]" aria-label="Open featured story studio"><FileText size={20} /></button>}
            {(viewer.role === 'worker' || viewer.role === 'volunteer') && <button onClick={() => setActivePanel('news')} className="p-2 rounded-full hover:bg-[#f6f4ef] text-[#66716a]" aria-label="Open newsroom"><Newspaper size={20} /></button>}
            <div className="h-8 w-px bg-[#d9d6ce] mx-2"></div>
            <div className="flex items-center gap-3">
              <div className="text-right hidden sm:block">
                <p className="text-xs font-black uppercase tracking-wider">{viewer.name}</p>
                <p className="text-[10px] font-bold text-[#66716a] uppercase">{viewer.role}</p>
              </div>
              <button onClick={() => openProfile(viewer.id, viewer.name)} className="relative h-10 w-10 rounded-full overflow-hidden border-2 border-[#e1ad45]" aria-label={`Open ${viewer.name} profile`}>
                <Image src={viewer.avatar} alt={viewer.name} fill className="object-cover" />
              </button>
              <button onClick={handleViewerLogout} className="p-2 rounded-full hover:bg-red-50 text-red-600" title="Logout" aria-label="Sign out">
                <LogOut size={20} />
              </button>
            </div>
          </div>
          ...
          <section onClick={(event) => event.stopPropagation()} className={`w-full rounded-3xl border border-[#d9d6ce] bg-white p-6 shadow-2xl ${activePanel === 'messages' || activePanel === 'newsletter' || activePanel === 'stories' || activePanel === 'news' ? 'max-w-4xl' : 'max-w-sm'}`}>
            ...
            {activePanel === 'notifications' && (
              <div className="mt-5 space-y-3">
                {viewer.role === 'worker' && messageUnreadCount > 0 ? (
                  <button onClick={() => setActivePanel('messages')} className="w-full rounded-2xl border border-[#e1ad45]/40 bg-[#fff8e8] p-4 text-left text-sm font-bold text-[#7a5b16] hover:bg-[#fff2cf]">You have {messageUnreadCount} unread contact message{messageUnreadCount === 1 ? '' : 's'}. Tap to read and reply.</button>
                ) : (
                  <button onClick={() => { setActivePanel(null); setNotice('You are up to date with the HMSI community feed.'); }} className="w-full rounded-2xl bg-[#f6f4ef] p-4 text-left text-sm hover:bg-[#e9f0e9]">You are up to date with the HMSI community feed.</button>
                )}
                <Link href="/opportunities" onClick={() => setActivePanel(null)} className="block rounded-2xl bg-[#f6f4ef] p-4 text-sm hover:bg-[#e9f0e9]">View volunteer opportunities</Link>
              </div>
            )}
            {activePanel === 'messages' && ((viewer.role === 'worker' || viewer.role === 'admin') ? <MessageInbox viewer={viewer.role === 'admin' ? { role: 'admin' } : { email: viewer.email, role: 'worker' }} compact onUnreadChange={setMessageUnreadCount} /> : <p className="mt-5 text-sm text-[#66716a]">Messages are available to approved workers and administrators.</p>)}
            {activePanel === 'newsletter' && ((viewer.role === 'worker' || viewer.role === 'volunteer') ? <NewsletterStudio viewer={{ email: viewer.email, name: viewer.name, role: viewer.role }} compact /> : <p className="mt-5 text-sm text-[#66716a]">Newsletter drafting is available to approved volunteers and workers.</p>)}
            {activePanel === 'stories' && ((viewer.role === 'worker' || viewer.role === 'volunteer') ? <FeaturedStoryStudio viewer={{ email: viewer.email, name: viewer.name, role: viewer.role }} /> : <p className="mt-5 text-sm text-[#66716a]">Featured story submissions are available to approved volunteers and workers.</p>)}
            {activePanel === 'news' && ((viewer.role === 'worker' || viewer.role === 'volunteer') ? <NewsroomStudio viewer={{ email: viewer.email, name: viewer.name, role: viewer.role }} /> : <p className="mt-5 text-sm text-[#66716a]">News submissions are available to approved volunteers and workers.</p>)}
          </section>
        </div>
      )}
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 grid grid-cols-1 lg:grid-cols-[280px_1fr_320px] gap-8">
          ...
          <nav className="bg-white rounded-3xl p-4 border border-[#d9d6ce] shadow-sm">
            <ul className="space-y-1">
              ...
              <li>
                ...
                {(viewer.role === 'worker' || viewer.role === 'admin') && <Link href="/worker-room" className="flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-bold text-[#66716a] transition-colors hover:bg-[#f6f4ef]"><MessageSquare size={20} /> Worker room</Link>}
              </li>
            </ul>
          </nav>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <button onClick={() => openProfile(activity.userId, activity.userName)} className="relative h-10 w-10 rounded-full overflow-hidden" aria-label={`Open ${activity.userName} profile`}>
                        <Image src={activity.userId === viewer.id ? viewer.avatar : "/images/outreach-4.png"} alt={activity.userName} fill className="object-cover" />
                      </button>
                      <div>
        ...
        <aside className="hidden xl:block space-y-6">
          <div className="bg-white rounded-3xl p-6 border border-[#d9d6ce] shadow-sm">
            ...
            <div className="space-y-4">
              {data.projects.flatMap((p: any) => p.tasks.filter((t: any) => t.assignedTo === viewer.id)).length > 0 ? (
                data.projects.flatMap((p: any) => p.tasks.filter((t: any) => t.assignedTo === viewer.id)).map((t: any) => (
