...
export async function GET(request: Request) {
  ...
  const assignments = await admin.from('work_assignments').select('id,title,description,kind,status,due_at,fundraiser_id,created_at,updated_at').eq('assigned_worker_id', session.workerId).order('due_at', { ascending: true, nullsFirst: false }).limit(100);
...
export async function PATCH(request: Request) {
  ...
  if (!id || !['assigned', 'in_progress', 'completed'].includes(status)) return NextResponse.json({ error: 'Assignment id and valid status are required.' }, { status: 400 });
  const update = await admin.from('work_assignments').update({ status, updated_at: new Date().toISOString() }).eq('id', id).eq('assigned_worker_id', session.workerId).select('id,status,updated_at').maybeSingle();
