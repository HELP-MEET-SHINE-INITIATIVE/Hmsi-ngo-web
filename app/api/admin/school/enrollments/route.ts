...
export async function POST(request: Request) {
  ...
  if (!holderRole || !holderId) return error('An active approved worker, volunteer, or member is required.');
  ...
  if (holderRole === 'worker') {
    ...
    if (result.error) return error('Member records are unavailable.', 503);
    if (!result.data || result.data.status !== 'active') return error('Only active approved members may be enrolled.');
    holder = result.data;
  }
