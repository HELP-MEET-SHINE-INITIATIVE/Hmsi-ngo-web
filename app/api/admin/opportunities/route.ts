...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { data, error } = await admin
      .from('opportunities')
      .insert({ title, description, audience, location, starts_at: new Date(startsAt).toISOString(), ends_at: endsAt && !Number.isNaN(Date.parse(endsAt)) ? new Date(endsAt).toISOString() : null, status, created_by: getAdminEmailFromCookie(request.headers.get('cookie')) })
